...
#define OUTAGE_TIME_BEFORE      (40 * 60 * CLOCK_SECOND)
#define OUTAGE_DURATION         (10 * 60 * CLOCK_SECOND)
#define OUTAGE_TIME_BETWEEN     (15 * 60 * CLOCK_SECOND)
...
receiver(struct simple_udp_connection *c,
         ...
         uint16_t datalen)
{
  printf("App: received, fp count %u", ((struct app_data *)data)->fpcount);
  rpl_trace((struct app_data *)data);
  /* Next printout is just for convenience when browsing logs, and placed after app printf to unnecessary extra delay */
  printf("Tcpip: for us, fp count %u", ((struct app_data *)data)->fpcount);
  rpl_trace((struct app_data *)data);
}
/*---------------------------------------------------------------------------*/
